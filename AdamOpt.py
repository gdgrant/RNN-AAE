...
class AdamOpt:
    ...
    def compute_gradients(self, loss, var_list=None, apply=True):
        vars = self.variables if var_list == None else var_list

        self.gradients = self.grad_descent.compute_gradients(loss, var_list=vars)

        self.t += 1
        lr = self.learning_rate*np.sqrt(1-self.beta2**self.t)/(1-self.beta1**self.t)
        self.update_var_op = []

        for (grads, _), var in zip(self.gradients, vars):
